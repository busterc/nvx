...
describe('nvx', () => {
  ...
  it('uses multiple versons as specified by arguments', () => {
    return nvx(['7.7', '4.2.1'], ['node', '-v']).then(results => {
      assert(results[0].command === 'npx -p node@7.7 node -v');
      assert(results[1].command === 'npx -p node@4.2.1 node -v');
    });
  });

  it('uses multiple versons as found in .travis.yml or circle.yml', () => {
    return nvx([], ['node', '-v']).then(results => {
      assert(results[0].command === 'npx -p node@6 node -v');
      assert(results[1].command === 'npx -p node@4 node -v');
    });
  });
